Feature: Measure 05 - Ratio Boolean Multi Component

  Background:
    * def measureId = '5RatioBooleanMultiComponent'

  Scenario: Evaluate and Validate Measure 05
    ...
    * def expectedReportRaw = read('classpath:expected-results/5RatioBooleanMultiComponentResponse.json')
    ...
    * eval if (hasStratifiers) { print(''); print('STRATIFIERS (from actual report):'); var strats = actualReport.group[0].stratifier; for (var i = 0; i < strats.length; i++) { var strat = strats[i]; if (strat.stratum) { for (var j = 0; j < strat.stratum.length; j++) { var stratum = strat.stratum[j]; if (stratum.component && stratum.component.length > 0) { var compValues = stratum.component.map(function(c) { return c.value && c.value.text ? c.value.text : 'unknown'; }).join(' | '); print('  Stratifier [' + compValues + ']:'); } else { var value = stratum.value && stratum.value.text ? stratum.value.text : 'unknown'; print('  Stratifier [' + value + ']:'); } if (stratum.population) { for (var k = 0; k < stratum.population.length; k++) { var pop = stratum.population[k]; var id = pop.id; var code = pop.code && pop.code.coding && pop.code.coding[0] ? pop.code.coding[0].code : 'unknown'; var count = pop.count; var display = id === code ? id : id + ' (' + code + ')'; print('    ' + display + ': ' + count); } } } } } }
